...
| 重要度 | 項目 | 内容 |
|---|---|---|
| 🔴 | EV検証なし | `parser.js` で `evs` 値の上限チェック(合計66 / 個別32)が無い。誤入力検出のため警告だけでも出すべき |
...
| 🟢 | マジックナンバー | `6`(パーティ最大/相手パーティ最大)、`4`(選出)、`3`(選出パターン行)、`12`(候補ポケモン)、`252` などが各所に直書き。`constants.js` に集約 |
...
| **EV合計バー表示** | 編集中に合計値が見えると入力ミスが激減(66超を即視認) | S |
| **パース時の警告**(合計66超/個別32超) | 入力品質ガードレール | S |
| **「同じパーティで記録」ボタン** | 既存「この構成で記録」の動線を強化、連戦時の入力時短 | S |
| **テキストエクスポート**(パーティ→Showdown形式) | 他ツール連携。コピペで `@smogon/calc` に流せる | S |
| **期間フィルタに「今週」追加** | 月曜起点の今週分のみ表示。週次の振り返り動線が即できる | S |

> ※ 上記 Phase 1 全6項目は 2026-05-10 実装済

### Phase 2（次にやる・中工数）

| 項目 | なぜやるか | 工数 |
|---|---|---|
| **`modal.js` の分割**（保留） | パーティ関連・対戦関連・サイドパネルを別ファイル化。今後の機能追加に備える | M |
| ~~**対戦記録の「テンプレ複製」機能**~~ ✅ | 直前の試合と相手だけ違うケースをワンクリックで | M |
| ~~**タグの色分け**~~ ✅ | 視覚的にカテゴリが分かる。フィルタ UX 向上 | M |
| ~~**個別ポケモンの統計ドリルダウン**~~ ✅ (既存実装あり) | ペア/トリオ統計から1体ドリルダウンができない | M |
| ~~**対戦予測**~~ ✅ (side panel に勝率表示済) | side-panel が既に「過去戦績」を出してるので拡張余地大 | M |
| ~~**CSVエクスポートに details 列追加**~~（取り下げ・各カードの📋コピーで代替） | パーティ単位で詳細を一括出力。Showdown形式互換 | S |

> ※ Phase 2 は `modal.js 分割` のみ未着手。残り5項目は 2026-05-10 実装済
