...
**注意: これは1対1の対面勝率ではない。** 「ガブリアスを選出して、相手パーティにサーフゴーがいた試合」では、ガブリアスとサーフゴーが直接対面したかどうかに関わらず、その試合全体の勝敗がカウントされる。選出判断の材料（「相手にYがいるときにXを出すとどうなるか」）として大まかな傾向を把握するのに使う。
